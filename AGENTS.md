...
- **Initial Sync Quietness**: Newly subscribed channels record the current timestamp upon addition; existing/past videos are never posted.
- **Periodic & On-demand RSS Polling**: Periodic checks run every 5 minutes (`CHECK_INTERVAL_SEC`), with on-demand checks available via `/reload` (polling channels checked > 5 minutes ago).
...
- Manages per-user records (`token`, `refresh_token`, `channels`).
- Tracks channel timestamps:
  - `last_published` in human format (`YYYY-MM-DD HH:MM:SS UTC`).
  - `last_checked` in human format (`YYYY-MM-DD HH:MM:SS UTC`).
- Handles subscription syncing: `sync_user_channels()` sets current timestamp for new channels so past videos are not alerted.
